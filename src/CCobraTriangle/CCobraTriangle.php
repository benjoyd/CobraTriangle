<?php
/**
 * Main class for CobraTriangle, holds everything.
 *
 * @package CobraTriangleCore
 */
class CCobraTriangle implements ISingleton {
...
public function FrontControllerRoute() {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath         = COBRATRIANGLE_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = COBRATRIANGLE_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl        = $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $ct->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $ct
    ...
    $ct = &$this;
    // First the default CobraTriangle themes/functions.php
    include(COBRATRIANGLE_INSTALL_PATH . '/themes/functions.php');
  ...
  /**
   * Draw HTML for a menu defined in $ct->config['menus'].
   *
