  ...
  public function Manage($action=null) {
    switch($action) {
      case 'install': 
        ...
        $this->db->ExecuteQuery(self::SQL('insert content'), array('home', 'page', 'Home page', "This is a demo page, this could be your personal home-page.\n\nCobraTriangle is based on Lydia. Lydia is a PHP-based MVC-inspired Content management Framework, watch the making of Lydia at: http://dbwebb.se/lydia/tutorial.", 'plain', $this->user['id']));
        $this->db->ExecuteQuery(self::SQL('insert content'), array('about', 'page', 'About page', "This is a demo page, this could be your personal about-page.\n\nCobraTriangle is based on Lydia. Lydia is used as a tool to educate in MVC frameworks.", 'plain', $this->user['id']));
        $this->db->ExecuteQuery(self::SQL('insert content'), array('download', 'page', 'Download page', "This is a demo page, this could be your personal download-page.\n\nCobraTriangle is on github https://github.com/benjoyd/CobraTriangle. Or you can download your own copy of lydia from https://github.com/mosbth/lydia.", 'plain', $this->user['id']));
        $this->db->ExecuteQuery(self::SQL('insert content'), array('bbcode', 'page', 'Page with BBCode', "This is a demo page with some BBCode-formatting.\n\n[b]Text in bold[/b] and [i]text in italic[/i] and a link [url=http://dbwebb.se]to dbwebb.se[/url]. You can also include images using bbcode, such as the CobraTriangle logo: [img]http://www.student.bth.se/~best13/phpmvc/kmom04/CobraTriangle/themes/core/logo_80x80.png[/img]", 'bbcode', $this->user['id']));
